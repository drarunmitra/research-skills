...
- Ship the **derived district/area-level aggregates** the figures actually use, with no row-level/identifiable cells. Confirm aggregation is contractually permitted before anything goes public.
...
- **Derived product** (e.g. boundaries dissolved/edited by the pipeline, with NO public URL): you cannot "link to the source". Instead **simplify it**. For spatial layers, topology-preserving simplification keeps adjacency:
  ```r
...
## Resources
- The Turing Way, a guide to reproducible, ethical, collaborative research: https://book.the-turing-way.org/
